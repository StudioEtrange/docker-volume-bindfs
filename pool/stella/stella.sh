...
usage() {
	...
	echo " L     exec -- <command> : execute a command in local current shell with all stella env var setted  (i.e ./stella.sh exec -- ls -al)"
	echo " L     boot shell <uri> : launch an interactive new shell with all stella env var setted inside an <uri>"
	echo " L     boot cmd <uri> -- <command> : execute a command inside an <uri> (i.e STELLA_LOG_STATE="OFF" ./stella.sh exec -- 'echo $STELLA_HOST_DEFAULT_IP')"
	echo " L     boot script <uri> --script=<script_path> [-- script arg]"
	echo " L 		for boot commands use 'local' as <uri> for current host :"
